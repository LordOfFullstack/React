...
class TodoList extends React.Component {
  ...
  componentDidUpdate() {
    let finishedItems = this.state.itemsCollection.filter(el => el.done === 'finished');

    let unfinishedItems = this.state.itemsCollection.filter(el => el.done === 'unfinished');
  ...
  handleItemsFilter = field => {
    ...
    let Items = field.filter(el=>el.important === 'Высокий');
  ...
  handleAddItem = item => {
    ...
    this.setState({
      ...
      if(this.props.route === 'all') {
        this.setState(prevState => {return { itemsToDisplay: prevState.itemsToDisplay.concat(item) };}, () => {
  ...
  handleItemDelete = item => {
    ...
    const newItems = this.state.itemsToDisplay.filter(item => item.id !== itemId);

    const newItemsCollection = this.state.itemsCollection.filter(item => item.id !== itemId);
  ...
  handleItemOutline = object => {
    ...
    let items = this.state.itemsCollection.filter(task => task.id === taskId);
  ...
  handleItemEdit = object => {
    ...
    let editedItem = this.state.itemsCollection.filter(item => item.id === itemId);
  ...
  handleItemSave = object => {
    ...
    let savedItem = this.state.itemsCollection.filter(item => item.id === itemId);
    ...
    setTimeout(() => {
      let finishedArray = this.state.itemsCollection.filter(el => el.done === 'finished');

      let unFinishedArray = this.state.itemsCollection.filter(el => el.done === 'unfinished');
  ...
  handleChangePriority = (item, e) => {
    let taskId = item.id;
    let newItem = this.state.itemsCollection.filter(task => task.id === taskId);
    ...
    this.setState({ itemsCollection: this.state.itemsCollection }, () => {
      ...
      //let importantItems = this.state.itemsCollection.filter(el => el.important === 'Высокий');
      let finishedArray = this.state.itemsCollection.filter(el => el.done === 'finished');
      let unFinishedArray = this.state.itemsCollection.filter(el => el.done === 'unfinished');
  ...
  sortFunction = sortItems => {
    ...
    if (filter) {
      let importantItems = sortItems.filter(el => el.important === 'Высокий');
  ...
  calendarFilter = selectedDate => {
    ...
    if (filter) {
      this.setState({ selectedDate: selectedDate }, ()=> {
        ...
        let itemDate = field.filter(item => item.date === selectedDate);
